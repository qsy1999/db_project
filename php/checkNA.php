...
$sql = "select result  from nucleic_acid_testing_result where nucleic_acid_testing_result.patient_ID = $targetID order by result_ID desc limit 2";
$result = $conn->query($sql);
$nuc = [];
...
if(sizeof($nuc) != 2 ||sizeof($tem) != 3 || sizeof($tre) != 1){
    ...
    $sql = "insert into message (patient_ID, to) values ('$targetID','$doctor_id_mild') ";
    $result = $conn->query($sql);
    if(!$result){
